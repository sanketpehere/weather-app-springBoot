...
Run tests from the inner `weather-app` folder:
...
- The `Client` implementation currently constructs a URL with a hard-coded API key and calls `https://api.weatherapi.com/v1/current.json`. For security and flexibility, move the API key and base URL to `application.properties` and inject them into `Client` (for example, using `@Value` or a configuration properties class).
- Add sample curl or HTTPie examples with expected response snippets for the most important endpoints.
- Consider adding integration tests that mock the external API (WireMock or MockRestServiceServer) so the client behavior is verified without calling the real API.

---

I updated the "Available Endpoints" section to match the controller in `src/main/java/com/miniproject/weather_app/controllers/WeatherController.java` and added notes about the hard-coded API key. If you'd like, I can also:

- Replace the hard-coded API key with a configuration property and update `Client.java` to read it from `application.properties` (I can make that change and run tests).
- Add an example curl response in the README.

Tell me which of those follow-ups you'd like me to implement next.
